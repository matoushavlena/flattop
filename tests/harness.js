...
;(function driver() {
  ...
  const placeAirborne = (x, y, a, spd) => {
    ...
    plane.turnT = 0; plane.elevT = 1;
    plane.towing = false; plane.groundTurnT = 0; plane.trapWire = null;
  };
  ...
  // 2a) miss the wires -> the crash barrier stops you, bruised but alive
  placeAirborne(carrier.x + 360, DECK_Y - 18, 0.085, 160);
  plane.throttle = 0.4;
  let barrierStopped = false, barrierSeen = false;
  for (let i = 0; i < 400; i++) {
    step(1);
    barrierSeen = barrierSeen || barrierFx > 0;
    if (game.state === "rearm") { barrierStopped = true; break; }
    if (!plane.alive) break;
  }
  assert(barrierStopped && barrierSeen && plane.alive, "barrier stopped the long landing, alive=" + plane.alive);
  game.rearmT = 0; keyDown("Enter"); keyUp("Enter"); step(2);

  // 2b) true bolter: touch down past the barrier, roll off the bow, stay alive
  placeAirborne(carrier.x + 470, DECK_Y - 18, 0.085, 160);
  plane.throttle = 0.4;
  ...
  // 3) trap: touch down in the wires at proper speed -> cable pays out -> rearm
  placeAirborne(carrier.x - 100, DECK_Y - 22, 0.12, 160);
  plane.throttle = 0.35;
  let trapped = false, wireSeen = false;
  for (let i = 0; i < 400; i++) {
    step(1);
    wireSeen = wireSeen || plane.trapWire != null;
  ...
  assert(wireSeen, "the arrestor cable visibly paid out during the trap");
  game.rearmT = 0; keyDown("Enter"); keyUp("Enter"); step(2);
  ...
  // --- v3 checks ---
  // 16) MG fire chews wooden buildings progressively
  buildLevel(0);
  game.state = "fly";
  placeAirborne(12000, 200, 0, 300);
  const barV = enemies.find(e => e.type === "barracks");
  const hpV = barV.hp;
  for (let i = 0; i < 10; i++) damageEnemy(barV, 0.12, "bullet");
  assert(barV.hp < hpV && !barV.dead, "MG fire damages barracks progressively, hp=" + barV.hp.toFixed(2));

  // 17) roll-turn reverses heading while staying level and upright
  placeAirborne(5000, SEA - 300, -0.05, 300);
  const yBefore = plane.y;
  keyDown("z"); keyUp("z"); step(50);
  assert(!plane.inv && Math.cos(plane.a) < 0 && plane.alive,
    "roll-turn reversed heading, a=" + plane.a.toFixed(2) + " inv=" + plane.inv);
  assert(Math.abs(plane.y - yBefore) < 60, "roll-turn stayed roughly level");

  // 18) landed planes are serviced on deck and towed aft — no elevator swap
  plane.onDeck = true; plane.elevT = 1; plane.turnT = 0;
  plane.x = carrier.x + 400; plane.speed = 0; plane.deckDir = 1; plane.a = 0;
  enterRearm(); step(2);
  game.rearmT = 0;
  keyDown("Enter"); keyUp("Enter"); step(2);
  assert(game.state === "fly" && plane.towing, "deck crew tows the serviced plane, towing=" + plane.towing);
  step(60 * 9);
  assert(!plane.towing && Math.abs(plane.x - (carrier.x + 92)) < 30,
    "towed back to the launch spot, x-off=" + Math.round(plane.x - carrier.x));

  // 19) replacement aircraft still ride the elevator up from the hangar
  resetPlaneOnDeck(); step(3);
  assert(game.state === "rearm", "fresh plane services in the hangar, state=" + game.state);
  game.rearmT = 0;
  keyDown("Enter"); keyUp("Enter"); step(2);
  assert(plane.elevT < 1, "elevator ride begins, elevT=" + plane.elevT.toFixed(2));
  step(160);
  assert(plane.elevT === 1, "elevator reached the flight deck");
  console.log("v3 checks passed (MG vs buildings, roll-turn, wire payout, tow-back, elevator)");
